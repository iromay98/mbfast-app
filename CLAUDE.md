...
- **見積りシミュレーター**: JP車両ページの価格表下に設置。施工メニュー(価格列→Woo可変商品のバリエーション)+オプション(固定価格→単品商品)を選ぶと合計が即時更新。価格は全て静的HTML(JS無効でも見える)。「この内容で申し込む」=選択分を順次カート投入→/cart/。「全ページを再反映」がJP商品更新→ページ生成の順で1ボタン実行(表示と課金のズレ防止)
