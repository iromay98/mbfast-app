...
- mbPIT: 施工記録→AI記事化→WordPress自動公開。`src/server/pit/`（pipeline/generate/guard/wordpress/images/vehicle/gamification）＋ `/hq/pit` 管理画面。本部投稿は `/hq/pit/post`（店舗選択・本店直営店舗=PitStore.dealerId null 可）。店舗マスタでカテゴリID空欄なら親545配下にWPカテゴリ自動作成。ガード: 排ガスデバイス無効化=held、音量系=注意書き。WPカテゴリ: 親545 / Charism=547 / On's=549 / Anubis=551 / プレジャー=553 / Glanzcoat=555。**既存の代理店カテゴリツリーには触れない**。記事はBRAND_BLOCK挿入＋本文にmbFAST禁止（mbPIT別ブランド運用）。顧客向け: `/mycar`（車検証QR→HMAC vehicle_key・車台番号は平文非保存）＋施工証明書 `/verify/[postId]?h=`（SERVER_SECRET必須・変更禁止）
