...
- **対応オプションの語彙はDB(`VehiclePageOption`)が単一の正**。追加・並べ替え・非表示・削除は /hq/prices の「対応オプションの項目を編集」から（コード変更不要）。`derivedFrom` に価格列keyを指定すると、その列に金額かASKがある車両は自動で〇（手動設定が優先）
- 価格表の「WordPressに反映」で、そのブランドの下書き・公開中の車両ページも自動更新される
